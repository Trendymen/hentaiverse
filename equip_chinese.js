// ==UserScript==
// @name      HV forums Hightlight Equipment CHN (2025)
...
// @author    ggxxsol(ggxxhy);mbbdzz;hc br;l13763824039
// @description zh-cn
// @version   2025.11.18
// ==/UserScript==
...
lklist = lklist.concat('Bazaar&ss=is')      //道具库1
lklist = lklist.concat('Character&ss=eq')  //装备2
lklist = lklist.concat('Bazaar&ss=es')    //装备库3
...
lklist = lklist.concat('net/itemlist') //Reasoning13
	...
	switch (temp){
      ...
      // @note: save the edit log and delete it from the innerHTML
      // var edit = tempequipment.innerHTML.match(/<span class=\"edit"\>.*/g) || '';
      // tempequipment.innerHTML=tempequipment.innerHTML.replace(/<span class=\"edit"\>.*/g,"");
      // @note: 下一行原作者是注释掉的, tempequipment.innerHTML=tempequipment.innerHTML.replace(/<span [^>]+>[^>]+>/g,"")
			// tempequipment.innerHTML=tempequipment.innerHTML.replace(/<span [^>]+>[^>]+>/g,"");
			// var tes = '<span style="color:#69f">\x3C!--/coloro-->Magnificent\x3C!--colorc--></span>\x3C!--/colorc--> \x3C!--coloro:#f00--><span style="color:#f00">\x3C!--/coloro-->Demonic\x3C!--colorc--></span>\x3C!--/colorc--> <b>Willow Staff</b> <b>of Destruction</b></a> (Lv.283, IW)(start:500k)';
			// tes.replace(/<span (?!edit.)+>[^>]+>/g,"");
      ...
      // adds it back
      // tempequipment.innerHTML=tempequipment.innerHTML.replace(/<\/[^>]+>/g,'')
      // tempequipment.innerHTML+=edit;
...
// 			//item()
// 			//tempequipment=yhanhua(torep,repby,tempequipment);
// 			document.getElementsByTagName('tr')[ii].innerHTML = tempequipment;
// 		}
		break;
        default:
	}
...
repby = repby.concat('HP全满,持续100回合2%的基础HP.')
torep = torep.concat('Provides a long-lasting mana restoration effect.')
repby = repby.concat('持续50回合回复1%的基础MP.')
...
repby = repby.concat('MP全满,持续100回复1%的基础MP.')
torep = torep.concat('Provides a long-lasting spirit restoration effect.')
repby = repby.concat('持续50回复1%的基础SP.')
...
repby = repby.concat('SP全满,持续100回合回复1%的基础SP.')
...
repby = repby.concat('当用在一件装备上时，会临时给予装备')
...
repby = repby.concat('如果你想受高等级怪物的青睐的话，请喂它们吃这种精致的食物吧')
...
repby = repby.concat('击败小猫娘后她送你的紫丁香。很显然这品种是她的最爱。')
...
repby = repby.concat('NO.5 萍琪的 1/10 比例缩放公仔。一位著名的杯子蛋糕与各式饼干糖果的行家。她只想让大家永远保持笑容。')
...
repby = repby.concat('NO.17 奥塔维亚的 1/10 比例缩放公仔。著名的大提琴家；据信创造了八度空间、八面体以及章鱼。')
...
repby = repby.concat('NO.27 甘米的等身大玩偶。是萍琪的忠实宠物。经常被发现潜伏在你的浴缸里。虽然技术上是只短吻鳄，但它仍然可以称得上是最棒的小马。')
//文物奖杯
torep = torep.concat('Precursor Artifact')
repby = repby.concat('古遗物')
...
repby = repby.concat('从被击败的冒险者身上收集来的材料')
torep = torep.concat('Required to upgrade equipment bonuses to')
repby = repby.concat('装备强化之材斧')
...
function eqmthh(eminn){
	try{
      ...
      em=eminn.innerHTML.match(/(Crude|Fair|Average|Fine|Super|Exquisite|Magnificent|Legendary|Peerless|Mag|Exq)(?:(?!<\/a>|<br>).)*(?=<\/a>|<br>)/g)
    }
    // @note: 有时不正确的regex，如果玩家有自己的color style就可能出错(/([>]|[>\[\]0-9A-Z- ]+)(Fine|Super|Exquisite|Average|Crude|Fair|Magnificent|Legendary|Peerless)[a-zA-Z- ]*/g)
	}
catch(e){}
...
//	}
    ...
    // @note original: tempeq=tempeq.replace(em[i],'>'+emc[i])
	...
	return tempeq
  // eq# are the part of equipment. Ex: eq1 = quality
  // replace all existing eq12345 to eq12345, instead of replace the whole content to eq1+eq2+eq3+eq4+eq5 format
function rep(temp, eqeq, eqc){
  temps=temp
	for(j=0;j<eqeq.length;j++){
    temps=temps.replaceAll(eqeq[j], eqc[j])
	}
  return temps
}
...
function eq5(){
    ...
    eqe5 = eqe5.concat('Great Mace');
    eqc5 = eqc5.concat('重锤');
    eqe5 = eqe5.concat('Estoc');
    eqc5 = eqc5.concat('刺剑（双）');
    eqe5 = eqe5.concat('Swordchucks');
    eqc5 = eqc5.concat('*锁链双剑（双）');
    //法杖
...
//盾或者材斧,武器不会出现这个
    ...
    eqc4 = eqc4.concat('<span style=\"background:#ffa500;color:#000000\" >力场盾</span>');
    //布甲
    ...
    eqe4 = eqe4.concat('Ironsilk');
    eqc4 = eqc4.concat('铁丝</span><span style=\"background:#FFFFFF;color:#000000\" >(布)</span>');
    eqe4 = eqe4.concat('Phase');
    eqc4 = eqc4.concat('<span style=\"background:#ffa500;color:#000000\" >相位</span><span style=\"background:#FFFFFF;color:#000000\" >(布)</span>');
    //轻甲
    eqe4 = eqe4.concat('Leather');
    eqc4 = eqc4.concat('皮革<span style=\"background:#FFCCFF;color:#000000\" >(轻)</span>');
    eqe4 = eqe4.concat('Kevlar');
    eqc4 = eqc4.concat('克维拉<span style=\"background:#FFCCFF;color:#000000\" >(轻)</span>');
    eqe4 = eqe4.concat('Drakehide');
    eqc4 = eqc4.concat('龙皮<span style=\"background:#FFCCFF;color:#000000\" >(轻)</span>');
    eqe4 = eqe4.concat('Shade');
    eqc4 = eqc4.concat('<span style=\"background:#ffa500;color:#000000\" >暗影</span><span style=\"background:#FFCCFF;color:#000000\" >(轻)</span>');
    //重甲
    eqe4 = eqe4.concat('Plate');
    eqc4 = eqc4.concat('板甲<span style=\"background:#6b06b4;color:#FFFFFF\" >(重)</span>');
    eqe4 = eqe4.concat('Chain');
    eqc4 = eqc4.concat('锁链<span style=\"background:#6b06b4;color:#FFFFFF\" >(重)</span>');
    eqe4 = eqe4.concat('Reactive');
    eqc4 = eqc4.concat('<span style=\"background:#6b06b4;color:#FFFFFF\" >反应</span><span style=\"background:#ffa500;color:black\" >(重)</span>');
    eqe4 = eqe4.concat('Power');
    eqc4 = eqc4.concat('<span style=\"background:#ffa500;color:#000000\" >动力</span><span style=\"background:#6b06b4;color:#FFFFFF\" >(重)</span>');
    //法杖
...
//防具后缀//🈲🚫Ⓓ◎☒🌈
    eqe3 = eqe3.concat('of the Ox');
    eqc3 = eqc3.concat('力量');
    eqe3 = eqe3.concat('of the Raccoon');
    eqc3 = eqc3.concat('灵巧');
    eqe3 = eqe3.concat('of the Cheetah');
    eqc3 = eqc3.concat('敏捷');
    eqe3 = eqe3.concat('of the Turtle');
    eqc3 = eqc3.concat('耐力');
    eqe3 = eqe3.concat('of the Fox');
    eqc3 = eqc3.concat('智力');
    eqe3 = eqe3.concat('of the Owl');
    eqc3 = eqc3.concat('感知');
    ...
    eqc3 = eqc3.concat('防火<span style=\"background:#f97c7c;color:#000000\" >☄</span>');
    eqe3 = eqe3.concat('of the Frost-born')
    eqc3 = eqc3.concat('防冰<span style=\"background:#94c2f5;color:#000000\" >☄</span>');
    eqe3 = eqe3.concat('of the Wind-waker')
    eqc3 = eqc3.concat('防风<span style=\"background:#51f04d;color:#000000\" >☄</span>');
    eqe3 = eqe3.concat('of the Thunder-child')
    eqc3 = eqc3.concat('防雷<span style=\"background:#fcff66;color:#000000\" >☄</span>');
    eqe3 = eqe3.concat('of the Thrice-blessed')
    eqc3 = eqc3.concat('防圣<span style=\"background:#FFFFFF;color:#000000\" >☄</span>');
    eqe3 = eqe3.concat('of the Spirit-ward')
    eqc3 = eqc3.concat('防暗<span style=\"background:#000000;color:#FFFFFF\" >☄</span>');
    ...
    eqc3 = eqc3.concat('<span style=\"color:red\" >格挡</span>')
...
//武器后缀//
    ...
    eqc3 = eqc3.concat('<span style=\"background:#a5a8cf\;color:#000000" >专注</span>')
    eqe3 = eqe3.concat('of Surtr')
    eqc3 = eqc3.concat('<span style=\"background:#f97c7c\;color:#000000" >史尔特尔</span>')
    eqe3 = eqe3.concat('of Niflheim')
    eqc3 = eqc3.concat('<span style=\"background:#94c2f5\;color:#000000" >尼福尔海姆</span>')
    eqe3 = eqe3.concat('of Mjolnir')
    eqc3 = eqc3.concat('<span style=\"background:#fcff66\;color:#000000" >妙尔尼尔</span>')
    eqe3 = eqe3.concat('of Freyr')
    eqc3 = eqc3.concat('<span style=\"background:#51f04d\;color:#000000" >弗雷</span>')
    eqe3 = eqe3.concat('of Heimdall')
    eqc3 = eqc3.concat('<span style=\"background:#ffffff\;color:#000000" >海姆达尔</span>')
...
//    eqc2 = eqc2.concat('Astral');
    ...
    eqc2 = eqc2.concat('<span style=\"background:#51f04d\;color:#000000" >风暴的</span>')
    eqe2 = eqe2.concat('Hallowed')
    eqc2 = eqc2.concat('<span style=\"background:#ffffff\;color:#000000" >圣光的</span>')
    eqe2 = eqe2.concat('Demonic')
    eqc2 = eqc2.concat('<span style=\"background:#000000\;color:#ffffff" >魔煞的</span>')
    eqe2 = eqe2.concat('Ethereal')
    eqc2 = eqc2.concat('<span style=\"background:#5c5a5a\;color:#ffffff" >虚空的</span>')
    eqe2 = eqe2.concat('Ruby');
    eqc2 = eqc2.concat('灭火<span style=\"background:#f97c7c;color:#000000\" >☄</span>');
    eqe2 = eqe2.concat('Cobalt');
    eqc2 = eqc2.concat('解冻<span style=\"background:#94c2f5;color:#000000\" >☄</span>');
    eqe2 = eqe2.concat('Jade');
    eqc2 = eqc2.concat('御风<span style=\"background:#51f04d;color:#000000\" >☄</span>');
    eqe2 = eqe2.concat('Amber');
    eqc2 = eqc2.concat('抗雷<span style=\"background:#fcff66;color:#000000\" >☄</span>');
    eqe2 = eqe2.concat('Zircon');
    eqc2 = eqc2.concat('蔽圣<span style=\"background:#FFFFFF;color:#000000\" >☄</span>');
    eqe2 = eqe2.concat('Onyx');
    eqc2 = eqc2.concat('驱暗<span style=\"background:#000000;color:#FFFFFF\" >☄</span>');
...
//    eqc1 = eqc1.concat('<span style=\"background:#f5b9cd\;color:#000000" >传奇</span>');
    ...
    eqc1 = eqc1.concat('<span style=\"background:#000000\;color:gold" >◐无双◑</span>');
    eqe1 = eqe1.concat('Exq');
    eqc1 = eqc1.concat('<span style=\"background:#6060f9\;color:#ffffff" >精致</span>');
    eqe1 = eqe1.concat('Mag');
    eqc1 = eqc1.concat('<span style=\"background:#0000ae\;color:#ffffff" >华丽</span>');

    // var peerlessF = '<span style=\"background:#00c\;color:#f00\">P</span>';
    // peerlessF += '<span style=\"background:#f00\;color:#f90\">e</span>';
    // peerlessF += '<span style=\"background:#f00\;color:#fc0\">e</span>'
    // peerlessF += '<span style=\"background:#f90\;color:#0c0\">r</span>';
    // peerlessF += '<span style=\"background:#fc0\;color:#09f\">l</span>';
    // peerlessF += '<span style=\"background:#0c0\;color:#00c\">e</span>';
    // peerlessF += '<span style=\"background:#09f\;color:#c0f\">s</span>';
    // peerlessF += '<span style=\"background:#00c\;color:#f00\">s</span>';#191B21
/*  var peerlessF = '<span style=\"color:#f00\">P</span>';
    peerlessF += '<span style=\"color:#f90\">e</span>';
    peerlessF += '<span style=\"color:#fc0\">e</span>'
    peerlessF += '<span style=\"color:#0c0\">r</span>';
    peerlessF += '<span style=\"color:#09f\">l</span>';
    peerlessF += '<span style=\"color:#00c\">e</span>';
    peerlessF += '<span style=\"color:#c0f\">s</span>';
    peerlessF += '<span style=\"color:#f00\">s</span>';
    eqc1 = eqc1.concat('<span style=\"background:#c761c9bf\;color:#000000" >' + peerlessF + '</span>');
*/
//  eqc1 = eqc1.concat('<span style=\"background:#9595957d\;color:#000000" >' + peerlessF + '</span>');
